    ...
    <div class="container">
        <div class="row mb-4">
            ...
            <div class="col-md-6 text-md-end">
                <div class="btn-group">
                    <a href="{{ route('calendar.index', ['date' => $prevMonth, 'view' => $view]) }}"
                        class="btn btn-outline-secondary">
                        <i class="fas fa-chevron-left"></i>
                    </a>
                    <button class="btn btn-outline-secondary" id="current-month">{{ $currentMonth }}</button>
                    <a href="{{ route('calendar.index', ['date' => $nextMonth, 'view' => $view]) }}"
                        class="btn btn-outline-secondary">
                        <i class="fas fa-chevron-right"></i>
                    </a>
                </div>
                <a href="{{ route('calendar.index', ['date' => now()->format('Y-m-d'), 'view' => $view]) }}"
                ...
                <div class="card">
                    <div class="card-header">
                        <ul class="nav nav-tabs card-header-tabs">
                            <li class="nav-item">
                                <a class="nav-link {{ $view == 'month' ? 'active' : '' }}"
                                    href="{{ route('calendar.index', ['view' => 'month', 'date' => $currentDate]) }}">เดือน</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link {{ $view == 'week' ? 'active' : '' }}"
                                    href="{{ route('calendar.index', ['view' => 'week', 'date' => $currentDate]) }}">สัปดาห์</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link {{ $view == 'day' ? 'active' : '' }}"
                                    href="{{ route('calendar.index', ['view' => 'day', 'date' => $currentDate]) }}">วัน</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link {{ $view == 'list' ? 'active' : '' }}"
                                    href="{{ route('calendar.index', ['view' => 'list', 'date' => $currentDate]) }}">รายการ</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link {{ $view == 'table' ? 'active' : '' }}"
                                    href="{{ route('calendar.index', ['view' => 'table', 'date' => $currentDate]) }}">ตารางห้อง</a>
                            </li>
                        </ul>
                    </div>
                    <div class="card-body">
                            ...
                            @foreach ($statusList as $status)
                                @if (!in_array($status['status_id'], [1, 2, 5, 6, 7]))
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div
                                            style="width: 20px; height: 20px; background-color: {{ $status['color'] }}; border-radius: 3px;">
                                        </div>
