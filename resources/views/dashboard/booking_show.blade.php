...
@section('content')
    <div class="container mt-4">
        ...
        <div class="card p-4">
            ...
            <p><strong>ค่าบริการรวม:</strong>
                {{ $booking->total_price !== null ? number_format($booking->total_price, 2) . ' บาท' : '-' }}
            </p>
            <p><strong>สถานะการชำระเงิน:</strong> {{ $booking->payment_status ?? '-' }}</p>
            <p><strong>บุคคลภายนอก:</strong>
                {{ $booking->is_external !== null ? ($booking->is_external ? 'ใช่' : 'ไม่ใช่') : '-' }}
            </p>
